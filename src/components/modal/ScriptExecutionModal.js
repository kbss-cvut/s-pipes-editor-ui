                        ...
                        <Table>
                            ...
                            {this.state.executions.map((data, key) => {
                                return (
                                    <tr key={key}>
                                        <td>{key}</td>
                                        <td onClick={() => this.handleNodeZoom(data[MODULE_URI])} style={{cursor: 'pointer'}}>{data[MODULE_URI].replace(/^.*[\\\/]/, '')}</td>
                                        ...
                                        <td>
                                            {data[MODULE_INPUT_PATH].map((d, k) => {return(
                                                <span key={"input"+k} onClick={() => window.open("/rest/file/download?file="+d["@id"], '_blank').focus()}  style={{cursor: 'pointer'}}>
                                        ...
                                        <td>
                                            {data[MODULE_OUTPUT_PATH].map((d, k) => {return(
                                                <span key={"output"+k} onClick={() => window.open("/rest/file/download?file="+d["@id"], '_blank').focus()}  style={{cursor: 'pointer'}}>
                                                    <FontAwesomeIcon icon={faDownload}/>
                                                </span>
                                            );})}
                                        </td>
                                        <td>
                                            {data[MODULE_VARIABLES].sort((a, b) => a[MODULE_VARIABLE_NAME].localeCompare(b[MODULE_VARIABLE_NAME])).map((d, k) => {return(
                                                <div key={"variable"+k}>
